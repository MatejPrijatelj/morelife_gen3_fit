...
def f_plot(sol0,fname="defname.png"):
    ...
    axes[0].set_ylabel("AST voltage[V], oxide coverage []")
    axes[0].set_xlabel("time [s]")
    
    axes[0].grid(True)
    axes2=axes[0].twinx()

    axes2.set_ylabel("Pt/Cu concentration[mol/l]")
...
#########################################################
#create initial dist and vec c vec l
...
dist_bins_om ,vec_c, vec_l=f_start_dist_alter(lmin=0.25,lmax=5,cmin=0.25,cmax=6,MPars=ModPars, loc_l=vek_in[0], scale_l=0.25, s_c=vek_in[1], loc_c=vek_in[2], scale_c=vek_in[3])
C,L = np.meshgrid(vec_c,vec_l)
...
FCstate = [funccHp,funcT,funcUPt_saw]
...
#import data and interpolate function for smoothing
#make bins
#######################################
bins_r=np.insert(vec_r+vec_r[0],0,0)
#import
# Load the XLS file
data_file_path = "g3c_input.xlsx"  # Update with your actual file path
df = pd.read_excel(data_file_path, engine="openpyxl",skiprows=1)  # Specify engine for old .xls format
...
t1=time.time()
print("time 3 cycs= "+str(t1-t0))
print(sol0.y[2,-1])

#plot cycle
#f_plot(sol0,fname="test_3_cyc.png")

#price function for eol
hist_EOL = np.histogram(nm*df.iloc[:,3].dropna().to_numpy()*0.5,bins=bins_r,density=True)
hist_fix_EOL=f_norm_dist1d(vec_r,f_fix_dist(hist_EOL[0]))
...
for i in vars:
    ...
    #construct bounds
    if i==14:
        bounds.append((ModPars[i]*0.95,ModPars[i]*1.05))
        initial_vars.append(ModPars[i])
    elif 0.5 < ModPars[i] <=1.5:
...
def f_price_EOL(vek_in):
    ...
    priceN = f_sum_abs( np.array(hist_fix_EOL),np.array(rez_norm_r),vec_r)
    price=priceN
...
current_date = datetime.now().strftime('%Y-%m-%d')
base_filename = f"dif_evo_3c{current_date}.txt"
...
def imported_main():
    ...
    #test cyc
    #sol,rez_X_4,rez_n_4,rez_stat_4 = f_test_cycle_skip(MC=3000,round_factor=0.8, round_max_f=0.2,pars=pars,X0=X0)
    #plt.plot(rez_n_4,rez_X_4[:,2])
    #plt.show()
    #stats=rez_n_4[:,-1]
    ...
    rezultat = differential_evolution(f_price_EOL, bounds,x0=vars_0,popsize=200, callback=f_callback, tol=0.0001, disp = True,updating="deferred",workers=16)
